...
class DistributedRunner(SingleRunner):
        ...
        if self.args.alt_style == "id_first":
            for alter in range(self.num_alternations):
                ...
                for id_epoch in range(self.args.id_epochs):
                    ...
                    batch_count= 0
                    # print(self.train_loader_id.sampler.dataset_task_size)
    ...
    def test(self, phase, path=None):
        ...
        for loader in self.testloaders:
            if self.test_filtered > 0:
                ...
                self.test_dataset_task(loader, phase)
    ...
    def test_dataset_task(self, testloader, phase, k_list=[5, 10, 20], flag=True):
        if self.rank == 0:
            logging.info(f'testing {testloader.dataset.dataset} dataset on {testloader.dataset.task} task')
        test_total = 0
        # Initialize metrics
        if flag:
            state_file = "rec_datasets/Amazon_M2/test_progress.pkl"
            try:
                with open(state_file, "rb") as f:
                    state = pickle.load(f)
                    count = state["batch_num"]
                    recalls = state["recalls"]
                    ndcgs = state["ndcgs"]
                logging.info(f"Resuming from batch {count}")
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(testloader)):
                ...
                prediction = self.model_rec.module.generate(
                        ...
                        max_length=50,
                        prefix_allowed_tokens_fn=prefix_allowed_tokens,
                        num_beams=max(k_list),
                        num_return_sequences=max(k_list),
                        output_scores=True,
                        return_dict_in_generate=True,
                    )
                ...
                prediction_ids = prediction["sequences"]
                prediction_scores = prediction["sequences_scores"]
                # logging.info(f'prediction_ids: {prediction_ids}')
                gold_sents = self.tokenizer.batch_decode(
                    output_ids, skip_special_tokens=True
                )
                # logging.info(f'gold_sents: {gold_sents}')
                generated_sents = self.tokenizer.batch_decode(
                    prediction_ids, skip_special_tokens=True
                )
                # logging.info(f'generated_sents: {generated_sents}')
                indices, targets = evaluate.rel_results(generated_sents, gold_sents, prediction_scores, max(k_list), phase, testloader.dataset.dataset)
                # logging.info(indices, targets)
                ...
                for k in k_list:
                    if len(indices) == 0:
                        # logging.info(f'len 0')
                        continue
                    preds_k = indices[:, :k]
                    # logging.info(f'preds_k: {preds_k}')
            ...
            metrics = {}
            for k in k_list:
                metrics[f'Recall@{k}'] = recalls[k] / len(testloader.dataset)
                metrics[f'NDCG@{k}'] = ndcgs[k] / len(testloader.dataset)
            
            if self.rank == 0:
                for k in k_list:
                    logging.info(f'Recall@{k}: {metrics[f"Recall@{k}"]}')
                    logging.info(f'NDCG@{k}: {metrics[f"NDCG@{k}"]}')
