...
class SingleRunner:
    ...
    def test_dataset_task(self, testloader, k_list=[5, 10, 20]):
        if self.rank == 0:
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(testloader):
                ...
                # Process each prediction and calculate metrics
                targets = output_ids[:, -1]
                for k in k_list:
                    preds_k = prediction_ids[:, :k]
            ...
            metrics = {}
            for k in k_list:
                metrics[f'Recall@{k}'] = recalls[k] / test_total
                metrics[f'NDCG@{k}'] = ndcgs[k] / test_total
