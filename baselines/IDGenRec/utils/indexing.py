...
def generative_indexing_id(data_path, dataset, user_sequence_dict, mode, phase=0):
    """
    Use generative indexing method to index the given user seuqnece dict.
    """

    user_index_file = os.path.join(data_path, dataset, f'user_generative_index_phase_{mode}_{phase}.txt')
    item_text_file = os.path.join(data_path, dataset, 'our_item_plain_text.txt')
    if mode == 'train':
        user_sequence_file = os.path.join(data_path, dataset, 'user_sequence_train.txt')
    elif mode == 'validation':
        user_sequence_file = os.path.join(data_path, dataset, 'user_sequence_valid.txt')
    elif mode == 'test':
        user_sequence_file = os.path.join(data_path, dataset, 'user_sequence_test.txt')
...
def generative_indexing_rec(data_path, dataset, user_sequence_dict, model_gen, tokenizer, mode, regenerate=True, phase=0):
    """
    Use generative indexing method to index the given user seuqnece dict.
    ...
    user_index_file = os.path.join(data_path, dataset, f'user_generative_index_phase_{mode}_{phase}.txt')  # TODO

    # generate item id file
    if not os.path.exists(item_index_file):
        logging.info(f"(re)generate textual id with id generator phase {phase}!")
    ...
    if not os.path.exists(user_index_file):
        logging.info(f"(re)generate user id with id generator mode {mode} phase {phase}!")
        generate_user_id_from_text(item_map, user_index_file, user_sequence_file, model_gen, tokenizer, data_path, dataset)
...
def get_dict_from_lines(lines):
    ...
    for line in lines:
        info = line.split(" ", 1)
        if len(info) != 2:
            print("Not info (get_dict_from_liens)", info)
            continue
...
def generate_item_id_from_text(item_text_file_dir, item_id_file_dir, model_gen, tokenizer, device="cpu"):
    ...
    model_gen.to(device)
    logging.info(f'id text generator, device: {device}')
...
def generate_user_id_from_text(item_map, user_index_file, user_sequence_file, model_gen, tokenizer, data_path, dataset):
    """
    item map: dictionary
    """
    ...
    tmp_file_dir = os.path.join(data_path, dataset, f"tmp_user_{user_index_file.split('_')[-2]}_{user_index_file.split('_')[-1].split('.')[0]}.txt")
    user_id_dict = {}
    user_in_file = []
    if os.path.exists(tmp_file_dir):
       with open(tmp_file_dir, 'r', encoding="utf-8") as tmp_file:
           for line in tmp_file:
               ...
               user_id_dict[id_] = text

    user_seq_dict = {}
    with open(user_sequence_file, 'r', encoding="utf-8") as file:
        for line in file:
            words = line.strip().split()
            if words:
                if words[0] not in user_id_dict:
    ...
    for uid, text in tqdm.tqdm(user_seq_dict.items()):
        ...
        if len(str(id)) == 0:
            id = 'Nothing'
        user_id_dict[uid] = str(id)
        if uid in user_id_dict:
           with open(tmp_file_dir, "a", encoding="utf-8") as tmp_file:
               tmp_file.write(f"{uid} {user_id_dict[uid]}\n")
