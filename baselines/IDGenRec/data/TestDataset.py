...
class TestDataset(Dataset):
    def __init__(self, args, dataset, task):
        ...
        self.user_sequence = utils.ReadLineFromFile(os.path.join(self.data_path, self.dataset, 'user_sequence_test.txt'))
...
class TestDatasetGen(Dataset):
    def __init__(self, args, dataset, task, model_gen, tokenizer, regenerate, phase):
        ...
        if self.item_indexing == 'sequential':
            ...
            self.reindex_user_seq_dict, self.item_map = indexing.generative_indexing_rec(self.data_path, self.dataset, self.user_sequence_dict, model_gen=model_gen, tokenizer=tokenizer, mode='test', regenerate=regenerate, phase=self.phase)
        else:
            raise NotImplementedError
