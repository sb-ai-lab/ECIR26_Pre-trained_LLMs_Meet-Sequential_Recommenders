    ...
    def __init__(self, args, dataset, mode, phase=0):
        ...
        if args.distributed:
            if self.item_indexing == 'sequential':
                ...
                self.reindex_user_seq_dict, self.item_map = indexing.generative_indexing_id(self.data_path, self.dataset, self.user_sequence_dict, self.mode, phase)
                if self.rank == 0:
                    logging.info("Reindex data with generative indexing method")
                    indexing.generative_indexing_rec(self.data_path, self.dataset, self.user_sequence_dict, None, None, mode=self.mode, regenerate=False, phase=phase)
                    dist.barrier()
                else:
                    dist.barrier()
                self.reindex_user_seq_dict_rec, self.item_map_rec = indexing.generative_indexing_rec(self.data_path, self.dataset, self.user_sequence_dict, None, None, mode=self.mode, regenerate=False, phase=phase)
            else:
