...
class CollatorGen:
    ...
    def __call__(self, batch):
        ...
        for length in hist_lengths:
            ...
            #print(f"padded_hist shape: {padded_hist.shape}")
            #print(f"current_index: {current_index}, length: {length}")

            padded_attention_mask = torch.zeros((max_hist_length, max_hist_token))

            if length == 0:
                #print(len(padded_hist))
                #print(history_inputs['input_ids'][current_index], current_index)
                padded_hist[0] = torch.tensor([0], dtype=torch.long)
                padded_attention_mask[0] = torch.tensor([0])
            else:
